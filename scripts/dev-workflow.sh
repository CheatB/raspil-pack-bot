#!/bin/bash
# Скрипт для автоматизации процесса разработки:
# изменения -> проверка -> push -> пересборка -> перезапуск

set -e

PROJECT_ROOT="/var/www/projects/raspil_pak"
cd "$PROJECT_ROOT"

echo "🔄 Dev workflow: изменения -> проверка -> push -> пересборка -> перезапуск"
echo ""

# 1. Проверка изменений
echo "📋 Проверяю изменения..."
if [ -z "$(git status --porcelain)" ]; then
    echo "✅ Нет изменений для коммита"
    exit 0
fi

git status --short

# 2. Спрашиваем подтверждение
read -p "Продолжить? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "❌ Отменено"
    exit 1
fi

# 3. Добавляем изменения
echo "📦 Добавляю изменения..."
git add -A

# 4. Коммит
echo "💾 Создаю коммит..."
read -p "Сообщение коммита: " commit_message
if [ -z "$commit_message" ]; then
    commit_message="Update: $(date +%Y-%m-%d\ %H:%M:%S)"
fi

git commit -m "$commit_message"

# 5. Push
echo "📤 Отправляю в git..."
git push origin main

# 6. Пересборка
echo "🔨 Пересобираю проект..."
pnpm -C apps/tg-bot build:prod
NODE_OPTIONS='--max-old-space-size=1536' pnpm -C apps/web build

# 7. Перезапуск PM2
echo "🔄 Перезапускаю процессы..."
pm2 restart emoji_bot_web
pm2 restart emoji_bot

echo ""
echo "✅ Готово! Изменения применены и процессы перезапущены."
echo ""
echo "Проверка статуса:"
pm2 list

